...
% [C(i_1);C(i_2)*A(i_1)/rho;...;C(i_n)*A(i_(n-1))...*A_1/rho^(n-1)] corresponding to a
% sequence (i_1,...,i_n).
...
% ex: ma([2,1],{eye(2),ones(2)},{[1,1],[1,0]},1)
function out =ma(s,A,C,rhoo)
s=flip(s); %s=(i_1,...,i_n)-->(i_n,...,i_1)
out=[]; %initialize the output matrix
for i=1:length(s)
    out=[out;mseq(s,A,C,rhoo)];%compute C(i_n)*A(i_(n-1))...*A_1
...
function out=mseq(s,A,C,rhoo)%for s=(i_n,...,i_1) compute C(i_n)*A(i_(n-1))...*A_1
out=C{s(1)};
if length(s)>1
for i=1:length(s)-1
    out=out*A{s(i+1)}/rhoo;%construct C(i_n)*A(i_(n-1))...*A_1 where n=length(s)
end
end
end
