...
theta=[randi([1 2])];
% for the slow switching signal comment out line 127 and uncomment out line
% 128.
%for i=1:N/4 theta=[theta I{1}(randi([1,size(I{1},1)]),:) randi([1,m])]; end
...
subplot(4,1,1);% Plot the switching signal \theta_t
stairs([0:N],theta)
axis([0 N 0.9 m+0.1])
xlabel('t')
ylabel('\theta(t)')

subplot(4,1,2);% Plot the switching signal \theta_t
stairs([0:N],kappa./[0:N])
xlabel('t')
ylabel('k(t)/t')

subplot(4,1,3);% Plot the automaton states q_t
stairs(0:N,categorical(st));
xlabel('t')
ylabel('q_{t}')

subplot(4,1,4); % Plot the Lyapunov function V(q_t,x_t)
