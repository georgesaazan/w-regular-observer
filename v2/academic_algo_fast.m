...
kappa=[0];
% Initialization of the trajector
traj1=[2;1.5;-0.5];
...
theta=[randi([1 2])];
% for the slow switching signal comment out line 162 and uncomment out line
% 161.
for i=1:N/4 theta=[theta O{randi([1,length(O)])} randi([1,m])]; end
...
% time=[0:N];
% hold on
% plot(time(accept),theta(accept),'o','MarkerSize',8,'LineWidth',2);
axis([0 N 0.9 m+0.1])
