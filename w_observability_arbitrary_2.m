...
k=5; %fik k the maximal length of observable sequences
A1=eye(3);A2=1.5*[0,0,1;0,1,0;1,0,0]; %define the sytem
C1=[1,0,0];C2=[0,1,1];
A={A1,A2};
C={C1,C2};
n=size(A1,2); %dimension of the system
...
s=0; %check if there isn't observable sequences to the specified k to end the program
for i=1:length(I)
s=s+isempty(I{i});
end
...
% To build our LMIs we must define a matrix per state and per transition
...
for l=2:size(I{i},2)
    ...
    for b=1:m
    variableY.(strcat('Y',q{end}(2:end),num2str(b))) = sdpvar(n,p);
    end
end
end
end
...
for i=1:length(q)
    for j=1:m
    x=q{i}(2:end); %For each state retain only the number part
    jp=num2str(j);
...
%% define gains L
% Define the gains using the poweret of [1,...,m].
for t=1:k
x={};
for i=1:t
    x=[x,1:m];
...
qu='Q0';
traj2=[2;1.5;-0.5];
V2=[sqrt(traj2'*variableQ.(qu)*traj2)];
for i=1:N
